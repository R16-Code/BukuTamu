...
<head>
    ...
    <title>Absen Keluar - <?php echo APP_NAME; ?></title>
...
<body class="bg-gray-50 min-h-screen py-8 px-4">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <!-- Step 1: Search -->
        <div id="searchSection" class="bg-white rounded-2xl shadow-professional-lg p-8">
            <div class="border-l-4 border-red-600 pl-4 mb-8">
                <h2 class="text-2xl font-bold text-gray-900">Cari Data Kunjungan</h2>
                <p class="text-gray-600 mt-1">Masukkan identitas untuk absen keluar</p>
            </div>
            ...
            <form id="searchForm" class="space-y-6">
                ...
                <button type="submit" id="searchBtn"
                        class="w-full bg-gradient-to-r from-red-600 to-red-700 text-white py-4 rounded-xl font-semibold text-lg hover:from-red-700 hover:to-red-800 transition shadow-lg">
                    Cari Data Kunjungan
                </button>
        ...
        <!-- Step 2: Exit Form -->
        <div id="exitSection" class="bg-white rounded-2xl shadow-professional-lg p-8" style="display: none;">
            <div class="border-l-4 border-red-600 pl-4 mb-8">
                <h2 class="text-2xl font-bold text-gray-900">Konfirmasi Absen Keluar</h2>
                <p class="text-gray-600 mt-1">Tanda tangan untuk menyelesaikan kunjungan</p>
            </div>
            
            <div class="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-6 mb-6 border border-gray-200">
                <h3 class="font-bold text-gray-900 mb-4 text-lg">Data Kunjungan Anda</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="bg-white p-3 rounded-lg">
                        ...
                        <p class="font-semibold text-gray-900" id="displayJamMasuk">-</p>
                    </div>
                    <div class="md:col-span-2 bg-white p-3 rounded-lg">
                        <p class="text-sm text-gray-600 mb-1">Keperluan</p>
                        <p class="font-semibold text-gray-900" id="displayKeperluan">-</p>
                    </div>
                </div>
            </div>
            ...
            <form id="exitForm" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        Tanda Tangan Keluar <span class="text-red-600">*</span>
                    </label>
                    <div class="border-2 border-gray-300 rounded-lg p-4 bg-gray-50">
                        <canvas id="signatureCanvasExit" class="signature-canvas w-full h-48 bg-white rounded"></canvas>
                        <button type="button" id="clearSignatureExit" 
                                class="mt-3 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition text-sm font-medium">
                            Hapus Tanda Tangan
                        </button>
                    </div>
                    <input type="hidden" id="tandaTanganKeluarData" name="tanda_tangan_keluar">
                </div>

                <div class="flex gap-3">
                    <button type="button" id="cancelBtn"
                            class="flex-1 bg-gray-600 text-white py-2.5 px-4 rounded-lg font-medium text-sm hover:bg-gray-700 transition">
                        Kembali
                    </button>
                    <button type="submit" id="exitSubmitBtn"
                            class="flex-1 bg-gradient-to-r from-red-600 to-red-700 text-white py-2.5 px-4 rounded-lg font-medium text-sm hover:from-red-700 hover:to-red-800 transition shadow-md">
                        Kirim Absen Keluar
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('searchForm').addEventListener('submit', async function(e) {
                ...
                searchBtn.innerHTML = '<span class="loading-spinner mr-2"></span> Mencari...';
                
                try {
                    const response = await fetch('api/find_entry.php', {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    if (result.success) {
                        currentVisitData = result.data;
                        displayVisitData(result.data);
                        document.getElementById('searchSection').style.display = 'none';
                        document.getElementById('exitSection').style.display = 'block';
                        
                        setTimeout(() => {
                            signatureCanvasExit = new SignatureCanvas('signatureCanvasExit', 'clearSignatureExit');
                        }, 100);
                        
                        window.scrollTo({ top: 0, behavior: 'smooth' });
                    } else {
                        showModal('error', 'Data Tidak Ditemukan', result.message);
                    }
                } catch (error) {
            ...
            document.getElementById('exitForm').addEventListener('submit', async function(e) {
                ...
                if (signatureCanvasExit.isEmpty()) {
                    showModal('error', 'Validasi Gagal', 'Tanda tangan keluar wajib diisi!');
                    return;
                }
                
                document.getElementById('tandaTanganKeluarData').value = signatureCanvasExit.getDataURL();
                ...
                try {
                    const response = await fetch('process_exit.php', {
                        method: 'POST',
                        body: formData
                    });
        ...
        function displayVisitData(data) {
            ...
            document.getElementById('displayFungsi').textContent = data.fungsi;
            document.getElementById('displayJamMasuk').textContent = data.jam_masuk_formatted;
            document.getElementById('displayKeperluan').textContent = data.keperluan;
        }
